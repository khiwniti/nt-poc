...
async function verifyFleetTopology() {
  ...
  try {
    ...
    addResult(
      ...
      facilityCount === EXPECTED_FACILITIES ? 'PASS' : 'FAIL',
      `Found ${facilityCount}/${EXPECTED_FACILITIES} - Cities: ${cities}`
    );
    ...
    for (const row of stringsResult.rows) {
      ...
      addResult(
        'Topology',
        `${row.facility} strings`,
        isCorrect ? 'PASS' : 'FAIL',
        `${row.string_count}/${EXPECTED_STRINGS_PER_FACILITY} strings`
      );
    }
    ...
    for (const row of batteriesPerFacilityResult.rows) {
      ...
      addResult(
        'Topology',
        `${row.facility} batteries`,
        isCorrect ? 'PASS' : 'FAIL',
        `${row.battery_count}/${EXPECTED_BATTERIES_PER_FACILITY} batteries`
      );
    }
    ...
    addResult(
      'Topology',
      'Total strings fleet-wide',
      totalStrings === EXPECTED_TOTAL_STRINGS ? 'PASS' : 'FAIL',
      `${totalStrings}/${EXPECTED_TOTAL_STRINGS}`
    );

    addResult(
      'Topology',
      'Total batteries fleet-wide',
      totalBatteries === EXPECTED_TOTAL_BATTERIES ? 'PASS' : 'FAIL',
      `${totalBatteries}/${EXPECTED_TOTAL_BATTERIES}`
    );
  } catch (error) {
    addResult('Topology', 'Verification', 'FAIL', (error as Error).message);
  }
}
...
async function verifyStringConfiguration() {
  ...
  try {
    ...
    for (const row of stringTypesResult.rows) {
      ...
      addResult(
        'String Config',
        `${row.string_type} strings`,
        row.count === expected ? 'PASS' : 'FAIL',
        `${row.count}/${expected} strings`
      );
    }
    ...
    if (batteriesPerStringResult.rows.length === 0) {
      addResult(
        'String Config',
        'Batteries per string consistency',
        'PASS',
        `All ${EXPECTED_TOTAL_STRINGS} strings have ${EXPECTED_BATTERIES_PER_STRING} batteries`
      );
    } else {
      for (const row of batteriesPerStringResult.rows) {
        addResult(
          ...
          `${row.string_name}`,
          'FAIL',
          `Has ${row.battery_count} batteries (expected ${EXPECTED_BATTERIES_PER_STRING})`
        );
      }
    }
  } catch (error) {
    addResult('String Config', 'Verification', 'FAIL', (error as Error).message);
  }
}
  ...
  try {
    ...
    for (const row of specsResult.rows) {
      ...
      addResult(
        ...
        isCorrect ? 'PASS' : 'FAIL',
        `${row.count} batteries, ${parseFloat(row.avg_voltage).toFixed(1)}V, ${parseFloat(row.avg_capacity).toFixed(2)}kWh`
      );
    }
    ...
    addResult(
      ...
      duplicateSerials.rows.length === 0 ? 'PASS' : 'FAIL',
      duplicateSerials.rows.length === 0
    ...
    const layoutComplete =
      layout.with_position === layout.total && layout.with_color === layout.total;

    addResult(
      'Battery Config',
      '3D layout data',
      layoutComplete ? 'PASS' : 'FAIL',
      `${layout.with_position}/${layout.total} batteries have 3D positions`
    );
    ...
    addResult('Battery Config', 'Verification', 'FAIL', (error as Error).message);
  }
}
...
async function verifySimulatorScaling() {
  ...
  try {
    ...
    const batchResponse = await axios.post(
      `${SIMULATOR_URL}/api/sensors/readings/batch`,
    ...
    addResult(
      ...
      batchResponse.data.count === 10 && duration < 5000 ? 'PASS' : 'WARN',
      `${batchResponse.data.count} readings in ${duration}ms`
    );
    ...
    addResult(
      'Simulator',
      'Estimated full fleet polling time',
      estimatedFullFleetTime < 60000 ? 'PASS' : 'WARN',
      `~${estimatedSeconds}s for ${EXPECTED_TOTAL_BATTERIES} batteries`
    );
  } catch (error) {
    if (axios.isAxiosError(error)) {
      ...
      addResult('Simulator', 'Connectivity', 'FAIL', (error as Error).message);
    }
  }
}
...
async function verifyDataIngestionReadiness() {
  ...
  try {
    ...
    addResult(
      ...
      hypertableResult.rows[0].is_hypertable ? 'PASS' : 'FAIL',
      'sensor_readings configured for time-series'
    );
    ...
    console.log(`\n  Storage Analysis:`);
    console.log(
      `    Current: ${storageEstimate.rows[0].current_size} (${storageEstimate.rows[0].current_rows} rows)`
    );
    ...
    if (parseInt(coverage.batteries_with_data) > 0) {
      addResult(
        ...
        'Recent sensor data',
        'PASS',
        `${coverage.batteries_with_data} batteries, ${coverage.total_readings} readings, latest: ${new Date(coverage.latest_reading).toISOString()}`
      );
    } else {
      addResult(
        'Data Ingestion',
    ...
    addResult('Data Ingestion', 'Verification', 'FAIL', (error as Error).message);
  }
}
...
async function printProductionSummary() {
  ...
  console.log(
    `  • Total Strings: ${EXPECTED_TOTAL_STRINGS} (${EXPECTED_STRINGS_PER_FACILITY} per site)`
  );
  console.log(`  • Rectifier Strings: 27 (3 per site)`);
  console.log(`  • UPS Strings: 54 (6 per site)`);
  console.log(
    `  • Total Batteries: ${EXPECTED_TOTAL_BATTERIES} (${EXPECTED_BATTERIES_PER_FACILITY} per site)`
  );
