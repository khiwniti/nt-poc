...
async function verifyAllCounts() {
  ...
  try {
    // 1. Facilities count
    const facilitiesResult = await knex.raw<{ rows: CountResult[] }>(
    ...
    // 2. Zones count
    const zonesResult = await knex.raw<{ rows: CountResult[] }>('SELECT COUNT(*) FROM zones;');
    ...
    const batterySystemsResult = await knex.raw<{ rows: CountResult[] }>(
      'SELECT COUNT(*) FROM battery_systems;'
    );
    ...
    const healthyBatteriesResult = await knex.raw<{ rows: CountResult[] }>(
      'SELECT COUNT(*) FROM battery_systems WHERE health_score >= 80;'
    );
    ...
    const issuesResult = await knex.raw<{ rows: CountResult[] }>(
      'SELECT COUNT(*) FROM battery_systems WHERE health_score < 80;'
    );
    ...
    const sensorReadingsResult = await knex.raw<{ rows: CountResult[] }>(
    ...
    // 5. Alerts count
    const alertsResult = await knex.raw<{ rows: CountResult[] }>('SELECT COUNT(*) FROM alerts;');
    const alertsCount = parseInt(alertsResult.rows[0].count, 10);
    console.log('5. ALERTS');
    console.log(`   Total alerts: ${alertsCount}`);

    // 5a. Active alerts
    const activeAlertsResult = await knex.raw<{ rows: CountResult[] }>(
    ...
    const healthDistribution = await knex.raw<{
      rows: Array<{ range: string; count: string }>;
    }>(`
      SELECT 
        CASE 
    ...
    const alertSeverityDistribution = await knex.raw<{
      rows: Array<{ severity: string; count: string }>;
    }>(`
