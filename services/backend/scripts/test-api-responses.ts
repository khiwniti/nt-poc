  ...
  try {
    ...
    try {
      ...
      if (facilitiesResponse.ok) {
        const facilitiesData = (await facilitiesResponse.json()) as any;
        console.log('   Status:', facilitiesResponse.status);
        console.log('   Response structure:', JSON.stringify(facilitiesData, null, 2));
        console.log(
          '   Facilities count from API:',
          facilitiesData.total || facilitiesData.data?.length || 0
        );
      } else {
        console.log('   Status:', facilitiesResponse.status);
    ...
    try {
      ...
      if (batteryResponse.ok) {
        const batteryData = (await batteryResponse.json()) as any;
