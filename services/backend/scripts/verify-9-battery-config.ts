...
async function verifyDatabaseSchema() {
  ...
  try {
    ...
    addResult(
      'Database',
      'Facilities table',
      facilityCount === EXPECTED_FACILITY_COUNT ? 'PASS' : 'FAIL',
      `Found ${facilityCount}/${EXPECTED_FACILITY_COUNT} facilities`
    );
    ...
    addResult(
      'Database',
      'Zones table',
      zoneCount === EXPECTED_ZONE_COUNT ? 'PASS' : 'FAIL',
      `Found ${zoneCount}/${EXPECTED_ZONE_COUNT} zones`
    );
    ...
    addResult(
      ...
      batteryCount === EXPECTED_BATTERY_COUNT ? 'PASS' : 'FAIL',
      `Found ${batteryCount}/${EXPECTED_BATTERY_COUNT} batteries`
    );
    ...
    const all3DFieldsPresent =
      layoutFields.has_position_x === layoutFields.total &&
    ...
    addResult(
      'Database',
      '3D layout fields',
      all3DFieldsPresent ? 'PASS' : 'FAIL',
      `${layoutFields.has_position_x}/${layoutFields.total} batteries have 3D positions`
    );
    ...
    addResult(
      ...
      isHypertable ? 'PASS' : 'FAIL',
      'sensor_readings is hypertable'
    );
  } catch (error) {
    addResult('Database', 'Schema verification', 'FAIL', (error as Error).message);
  }
}
  ...
  try {
    ...
    if (batteryResult.rows.length > 0) {
      ...
      addResult(
        ...
        `Battery ${batteryId}: voltage=${readingResponse.data.voltage}V`
      );
    }
    ...
    addResult(
      'Simulator',
      'Batch readings',
      batchResponse.data.count === EXPECTED_BATTERY_COUNT ? 'PASS' : 'FAIL',
      `Received ${batchResponse.data.count}/${EXPECTED_BATTERY_COUNT} readings`
    );
  } catch (error) {
...
async function verifySensorData() {
  ...
  try {
    ...
    for (const row of recentReadingsResult.rows) {
      ...
      if (hasRecentData) batteriesWithRecentData++;

      addResult(
        'Sensor Data',
        `${row.serial_number}`,
        hasRecentData ? 'PASS' : 'FAIL',
        hasRecentData
    ...
    addResult(
      'Sensor Data',
      'Coverage',
      batteriesWithRecentData === EXPECTED_BATTERY_COUNT ? 'PASS' : 'FAIL',
      `${batteriesWithRecentData}/${EXPECTED_BATTERY_COUNT} batteries have recent data`
    );
  } catch (error) {
    addResult('Sensor Data', 'Verification', 'FAIL', (error as Error).message);
  }
}
...
async function verifyBatteryConfiguration() {
  ...
  try {
    ...
    for (const row of distributionResult.rows) {
      addResult(
        'Battery Config',
        `${row.facility_name} - ${row.zone_name}`,
        row.battery_count > 0 ? 'PASS' : 'FAIL',
        `${row.battery_count} batteries`
      );
    }
    ...
    addResult(
      ...
      duplicateSerialResult.rows.length === 0 ? 'PASS' : 'FAIL',
      duplicateSerialResult.rows.length === 0
