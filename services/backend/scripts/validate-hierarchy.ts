...
async function validateHierarchy() {
  try {
    ...
    facilities.forEach((f: any) => console.log(`   - ${f.name} (${f.id})`));

    // Check zones
    const zones = await knex('zones').select('id', 'name', 'facility_id');
    console.log('\n🏢 Zones:', zones.length);
    zones.forEach((z: any) => console.log(`   - ${z.name} (facility: ${z.facility_id})`));
    ...
    batterySystems.forEach((b: any) => console.log(`   - ${b.serial_number} (zone: ${b.zone_id})`));
    ...
    alerts.slice(0, 3).forEach((a: any) => console.log(`   - ${a.severity} (${a.status})`));
    ...
    const zonesWithValidFacility = zones.filter((z: any) =>
      facilities.some((f: any) => f.id === z.facility_id)
    );
    console.log(
      '   - All zones reference valid facilities:',
      zonesWithValidFacility.length === zones.length ? '✓' : '✗'
    );

    // Verify all battery_systems have valid zone_id
    const systemsWithValidZone = batterySystems.filter((b: any) =>
      zones.some((z: any) => z.id === b.zone_id)
    );
    console.log(
      '   - All battery systems reference valid zones:',
