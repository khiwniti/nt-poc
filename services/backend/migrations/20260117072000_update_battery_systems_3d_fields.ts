...
export async function up(knex: Knex): Promise<void> {
  ...
  const [hasPosX, hasPosY, hasPosZ, hasPitch, hasYaw, hasRoll, hasWidth, hasHeight, hasDepth, hasRackId, hasBayPosition, hasDisplayColor, hasIconType, hasModel3dRef, hasInstallationDate, hasLastMaintenance] =
    await Promise.all([
  ...
  if (!hasWidth || !hasHeight || !hasDepth) {
    ...
    await knex.raw(
      `ALTER TABLE battery_systems ADD CONSTRAINT ${DIMENSION_CHECK} CHECK ((width_m IS NULL OR width_m > 0) AND (height_m IS NULL OR height_m > 0) AND (depth_m IS NULL OR depth_m > 0))`
    );
  }

  await knex.raw(
    `CREATE INDEX IF NOT EXISTS ${POSITION_INDEX} ON battery_systems (zone_id, status, position_x, position_y, position_z)`
  );
  ...
  await knex.raw(
    `DROP INDEX IF EXISTS ${RACK_BAY_UNIQUE}`
  );
  await knex.raw(
    `CREATE UNIQUE INDEX ${RACK_BAY_UNIQUE} ON battery_systems (zone_id, rack_id, bay_position) WHERE rack_id IS NOT NULL AND bay_position IS NOT NULL`
  );
}
