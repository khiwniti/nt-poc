...
export async function up(knex) {
    ...
    const [hasFloorLevel, hasWidth, hasLength, hasHeight, hasBoundary, hasMaxCapacity, hasFloorPlanUrl, hasLayoutType] = await Promise.all([
    ...
    if (!hasLayoutType) {
        ...
        await knex.raw(`ALTER TABLE zones ADD CONSTRAINT ${LAYOUT_TYPE_CONSTRAINT} CHECK (layout_type = ANY(ARRAY['rack','cabinet','floor']))`);
    }
    if (!hasBoundary) {
        await knex.raw(`ALTER TABLE zones DROP CONSTRAINT IF EXISTS ${BOUNDARY_JSON_CONSTRAINT}`);
        await knex.raw(`ALTER TABLE zones ADD CONSTRAINT ${BOUNDARY_JSON_CONSTRAINT} CHECK (boundary_coordinates IS NULL OR jsonb_typeof(boundary_coordinates) = 'array')`);
    }
    await knex.raw(`CREATE INDEX IF NOT EXISTS ${FACILITY_FLOOR_INDEX} ON zones (facility_id, floor_level)`);
    await knex.raw(`CREATE INDEX IF NOT EXISTS ${BOUNDARY_GIN_INDEX} ON zones USING GIN (boundary_coordinates jsonb_path_ops)`);
}
