...
export async function up(knex) {
    ...
    if (!featureStoreExists) {
        ...
        await knex.raw(`COMMENT ON TABLE ${FEATURE_STORE_TABLE} IS 'Feature aggregates for ML pipelines. Consider converting to Timescale hypertable externally.'`);
    }
    ...
    if (!predictionHistoryExists) {
        ...
        await knex.raw(`ALTER TABLE ${PREDICTION_HISTORY_TABLE} ADD CONSTRAINT ${PREDICTION_TYPE_CHECK} CHECK (prediction_type IN ('rul','anomaly','failure_mode'))`);
        await knex.raw(`CREATE INDEX IF NOT EXISTS idx_prediction_history_feature_snapshot_gin ON ${PREDICTION_HISTORY_TABLE} USING GIN (feature_values_snapshot)`);
        await knex.raw(`CREATE INDEX IF NOT EXISTS idx_prediction_history_metadata_gin ON ${PREDICTION_HISTORY_TABLE} USING GIN (prediction_metadata)`);
    }
    ...
    if (!modelDriftExists) {
        ...
        await knex.raw(`ALTER TABLE ${MODEL_DRIFT_TABLE} ADD CONSTRAINT ${DRIFT_TYPE_CHECK} CHECK (drift_type IN ('data_drift','concept_drift','prediction_drift'))`);
    }
