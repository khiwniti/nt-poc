...
export async function up(knex: Knex): Promise<void> {
  ...
  if (!sensorsExists) {
    ...
    await knex.raw(
      `ALTER TABLE ${SENSORS_TABLE} ADD CONSTRAINT ${SENSOR_STATUS_CHECK} CHECK (status IN ('active','inactive','faulty','calibrating'))`
    );
  }
  ...
  if (!eventLogExists) {
    ...
    await knex.raw(
      `ALTER TABLE ${SIMULATION_EVENT_LOG_TABLE} DROP CONSTRAINT IF EXISTS ${EVENT_TYPE_CHECK}`
    );
    await knex.raw(
      `ALTER TABLE ${SIMULATION_EVENT_LOG_TABLE} ADD CONSTRAINT ${EVENT_TYPE_CHECK} CHECK (event_type IN ('scenario_changed','failure_injected','sensor_calibrated','config_updated'))`
    );

    await knex.raw(
      `CREATE INDEX IF NOT EXISTS idx_simulation_event_log_previous_state_gin ON ${SIMULATION_EVENT_LOG_TABLE} USING GIN (previous_state)`
    );
    await knex.raw(
      `CREATE INDEX IF NOT EXISTS idx_simulation_event_log_new_state_gin ON ${SIMULATION_EVENT_LOG_TABLE} USING GIN (new_state)`
    );
  }
}
