...
export class AlertEscalationService {
   ...
   */
  async getEscalationRule(facilityId: string): Promise<EscalationRule | null> {
  ...
  private getNextSeverity(currentSeverity: AlertSeverity): AlertSeverity | null {
    const escalationPath: Record<AlertSeverity, AlertSeverity | null> = {
      [AlertSeverity.INFO]: AlertSeverity.MEDIUM,
  ...
  private getEscalationMinutes(
    ...
    const timeframes: Record<AlertSeverity, number | null> = {
      [AlertSeverity.INFO]: rule.infoToMediumMinutes,
  ...
  async findEscalationCandidates(): Promise<EscalationCandidate[]> {
    ...
    try {
      ...
      for (const alert of alertsResult.rows) {
        ...
        if (escalationMinutes === null) continue;
  ...
  async escalateAlert(
    ...
    try {
      ...
      // Get current alert
      const alertResult = await client.query<Alert>(
        `SELECT 
          id,
          battery_system_id as "batterySystemId",
          zone_id as "zoneId",
      ...
      const eventResult = await client.query<EscalationEvent>(
        ...
        [alertId, fromSeverity, toSeverity, reason]
      );
   ...
   */
  private mapAlertSeverityToEmailSeverity(
    severity: AlertSeverity
  ...
  async processEscalations(): Promise<{
    ...
    try {
      ...
      for (const candidate of candidates) {
        try {
          // Escalate the alert
          const event = await this.escalateAlert(
            candidate.alert.id,
            candidate.targetSeverity,
            `Auto-escalated after ${Math.floor(candidate.minutesSinceCreated)} minutes without acknowledgement`
          );
          ...
          const errorMsg = `Failed to escalate alert ${candidate.alert.id}: ${
      ...
      const errorMsg = `Failed to process escalations: ${
        error instanceof Error ? error.message : String(error)
      }`;
   ...
   */
  async getEscalationHistory(alertId: string): Promise<EscalationEvent[]> {
    const client = await pool.connect();
    try {
      const result = await client.query<EscalationEvent>(
        `SELECT 
          id,
          alert_id as "alertId",
          from_severity as "fromSeverity",
  ...
  async upsertEscalationRule(
    ...
    const client = await pool.connect();
    try {
      const result = await client.query<EscalationRule>(
