...
export class PredictiveMaintenanceModel {
   ...
   */
  private extractFeatures(features: MaintenanceFeatures): number[] {
    return [
      features.sohDelta,
   ...
   */
  private encodeLabel(riskLevel: RiskLevel): number {
   ...
   */
  private decodeLabel(label: number): RiskLevel {
    const mapping: Record<number, RiskLevel> = {
      0: 'safe',
      1: '30d',
      2: '14d',
      3: '7d',
   ...
   */
  private async calculateMetrics(validationData: TrainingData[]): Promise<ModelMetrics> {
    if (!this.model) {
   ...
   */
  private calculateSimplifiedAUC(
    yTrue: number[],
    yPred: number[],
    targetClass: number
  ): number {
    ...
    for (let i = 0; i < yTrue.length; i++) {
      ...
      if (isPositive && predictedPositive) truePositives++;
      else if (!isPositive && predictedPositive) falsePositives++;
      else if (!isPositive && !predictedPositive) trueNegatives++;
      else falseNegatives++;
    }
   ...
   */
  private getDefaultProbabilities(riskLevel: RiskLevel): number[] {
    const defaults: Record<RiskLevel, number[]> = {
...
// Singleton instance
let modelInstance: PredictiveMaintenanceModel | null = null;

/**
 * Get the trained model instance
 */
export function getModel(): PredictiveMaintenanceModel {
  if (!modelInstance) {
 ...
 */
export async function initializeModel(trainingData?: TrainingData[]): Promise<void> {
 ...
 */
function generateDefaultTrainingData(): TrainingData[] {
