...
router.get('/metrics', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existingResult = await pool.query(
      ...
      [batterySystemId, modelVersion, start, end, aggregationPeriod]
    );
    ...
    const metrics = await ModelPerformanceService.calculatePerformanceMetrics(
      batterySystemId as string,
      start,
      end,
      modelVersion as string,
      aggregationPeriod as 'hourly' | 'daily' | 'weekly'
    );
 ...
 */
router.get('/drift', async (req: AuthRequest, res: Response) => {
  try {
    const {
      batterySystemId,
      baselineStart,
    ...
    const driftResult = await ModelPerformanceService.detectDrift(
      batterySystemId as string,
      new Date(baselineStart as string),
      new Date(baselineEnd as string),
      new Date(comparisonStart as string),
      new Date(comparisonEnd as string),
      modelVersion as string,
  ...
  try {
    ...
    const existingResult = await pool.query(
      `SELECT
       ...
       ORDER BY metric_time DESC`,
      [batterySystemId, start, end, aggregationPeriod]
    );
    ...
    const qualityMetrics = await ModelPerformanceService.calculateDataQualityMetrics(
      batterySystemId as string,
      start,
      end,
      aggregationPeriod as 'hourly' | 'daily' | 'weekly'
  ...
  try {
    ...
    const existingResult = await pool.query(
      `SELECT
       ...
       LIMIT 1`,
      [batterySystemId, modelVersion, start]
    );
    ...
    const accuracyMetrics = await ModelPerformanceService.calculatePerformanceMetrics(
      batterySystemId as string,
      start,
      end,
      modelVersion as string,
    ...
    const driftResult = await ModelPerformanceService.detectDrift(
      batterySystemId as string,
      new Date(start.getTime() - 7 * 24 * 60 * 60 * 1000), // 1 week before
      start,
      start,
      end,
      modelVersion as string
    ...
    const dataQuality = await ModelPerformanceService.calculateDataQualityMetrics(
      batterySystemId as string,
      start,
      end,
      'daily'
    ...
    // Store health score
    await pool.query(
      `INSERT INTO model_health_scores
    ...
    await ModelPerformanceService.checkAndCreateAlerts(
      batterySystemId as string,
      modelVersion as string,
      healthScore,
      accuracyMetrics,
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT
       ...
       LIMIT $3`,
      [batterySystemId, isResolved, parseInt(limit as string)]
    );
 ...
 * Resolve an alert
 */
router.patch('/alerts/:id/resolve', async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
