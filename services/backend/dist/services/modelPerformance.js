...
export class ModelPerformanceService {
     ...
     */
    static calculateR2(predicted, actual) {
        if (predicted.length !== actual.length || predicted.length === 0) {
            throw new Error('Arrays must be non-empty and of equal length');
        }
     ...
     */
    static calculateKSStatistic(baseline, comparison) {
     ...
     */
    static async calculatePerformanceMetrics(batterySystemId, startTime, endTime, modelVersion, aggregationPeriod) {
       ...
       WHERE battery_system_id = $1
         ...
         AND actual_soc IS NOT NULL`, [batterySystemId, modelVersion, startTime, endTime]);
        ...
        const missingResult = await pool.query(`SELECT COUNT(*) as missing_count
       FROM model_predictions
       WHERE battery_system_id = $1
         AND model_version = $2
         AND prediction_time >= $3
         AND prediction_time <= $4
         AND actual_soc IS NULL`, [batterySystemId, modelVersion, startTime, endTime]);
     ...
     */
    static async detectDrift(batterySystemId, baselineStart, baselineEnd, comparisonStart, comparisonEnd, modelVersion, driftThreshold = 0.2) {
        // Fetch baseline data
        const baselineResult = await pool.query(`SELECT voltage, current, temperature, soc
       FROM sensor_readings
       WHERE battery_system_id = $1
         AND time >= $2
         AND time <= $3
       ORDER BY time`, [batterySystemId, baselineStart, baselineEnd]);
        // Fetch comparison data
        const comparisonResult = await pool.query(`SELECT voltage, current, temperature, soc
       FROM sensor_readings
       WHERE battery_system_id = $1
         AND time >= $2
         AND time <= $3
       ORDER BY time`, [batterySystemId, comparisonStart, comparisonEnd]);
        ...
        const voltageDrift = this.calculateKSStatistic(baseline.map(r => parseFloat(r.voltage)), comparison.map(r => parseFloat(r.voltage)));
        const currentDrift = this.calculateKSStatistic(baseline.map(r => parseFloat(r.current)), comparison.map(r => parseFloat(r.current)));
        const temperatureDrift = this.calculateKSStatistic(baseline.map(r => parseFloat(r.temperature)), comparison.map(r => parseFloat(r.temperature)));
        const socDrift = this.calculateKSStatistic(baseline.map(r => parseFloat(r.soc)), comparison.map(r => parseFloat(r.soc)));
     ...
     */
    static async calculateDataQualityMetrics(batterySystemId, startTime, endTime, aggregationPeriod) {
       ...
       WHERE battery_system_id = $1
         AND time >= $2
         AND time <= $3
       ORDER BY time`, [batterySystemId, startTime, endTime]);
     ...
     */
    static calculateHealthScore(accuracyMetrics, driftScore, dataQualityMetrics) {
        // Accuracy score (based on R² values, 0-100 scale)
        const avgR2 = (accuracyMetrics.r2.soc +
            accuracyMetrics.r2.soh +
            accuracyMetrics.r2.temperature +
            accuracyMetrics.r2.power) / 4;
        ...
        const totalMissing = Object.values(dataQualityMetrics.missingCounts).reduce((sum, count) => sum + count, 0);
        const totalOutliers = Object.values(dataQualityMetrics.outlierCounts).reduce((sum, count) => sum + count, 0);
        const totalViolations = Object.values(dataQualityMetrics.rangeViolations).reduce((sum, count) => sum + count, 0);
        ...
        const dataQualityScore = Math.max(0, Math.min(100, (1 - missingRate - outlierRate - violationRate) * 100));
        // Overall health score (weighted average)
        const overallHealthScore = (accuracyScore * 0.5 +
     ...
     */
    static async checkAndCreateAlerts(batterySystemId, modelVersion, healthScore, accuracyMetrics, driftDetected, driftScore) {
        const alerts = [];
        ...
        // Check R² values
        Object.entries(accuracyMetrics.r2).forEach(([metric, value]) => {
            if (value < 0.7) {
                alerts.push({
                    type: 'accuracy_degradation',
        ...
        for (const alert of alerts) {
            await pool.query(`INSERT INTO model_health_alerts
