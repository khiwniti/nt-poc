...
describe('AlertEscalationService', () => {
    ...
    describe('upsertEscalationRule', () => {
        it('should create a new escalation rule', async () => {
            const rule = await alertEscalationService.upsertEscalationRule('new-facility', 100, 50, 25, true);
        ...
        it('should update existing escalation rule', async () => {
            ...
            const updatedRule = await alertEscalationService.upsertEscalationRule('update-facility', 80, 40, 20, false);
    ...
    describe('findEscalationCandidates', () => {
        it('should find high severity alerts eligible for escalation to critical', async () => {
          ...
          ) VALUES ($1, $2, $3, $4, $5, $6, $7)`, ['battery-1', 'default', 'Temperature High', 'high', 'active', 'High temperature detected', createdAt]);
        ...
        it('should find medium severity alerts eligible for escalation to high', async () => {
            ...
            try {
                ...
                await client.query(`INSERT INTO alerts (
            battery_system_id, facility_id, type, severity, status, message, created_at
          ) VALUES ($1, $2, $3, $4, $5, $6, $7)`, ['battery-1', 'default', 'Voltage Anomaly', 'medium', 'active', 'Voltage anomaly detected', createdAt]);
                const candidates = await alertEscalationService.findEscalationCandidates();
                expect(candidates).toHaveLength(1);
        ...
        it('should not find alerts that are acknowledged', async () => {
          ...
          ) VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`, ['battery-1', 'default', 'Temperature High', 'high', 'acknowledged', 'High temperature detected', createdAt]);
                const candidates = await alertEscalationService.findEscalationCandidates();
        ...
        it('should not find critical alerts (cannot escalate beyond critical)', async () => {
            ...
            try {
                ...
                await client.query(`INSERT INTO alerts (
            battery_system_id, facility_id, type, severity, status, message, created_at
          ) VALUES ($1, $2, $3, $4, $5, $6, $7)`, ['battery-1', 'default', 'Critical Failure', 'critical', 'active', 'Critical failure detected', createdAt]);
                const candidates = await alertEscalationService.findEscalationCandidates();
                expect(candidates).toHaveLength(0);
            }
            finally {
    ...
    describe('escalateAlert', () => {
        it('should escalate alert and create escalation event', async () => {
            ...
            try {
                ...
                const result = await client.query(`INSERT INTO alerts (
            battery_system_id, facility_id, type, severity, status, message
          ) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`, ['battery-1', 'default', 'Temperature High', 'high', 'active', 'High temperature detected']);
                alertId = result.rows[0].id;
                // Escalate the alert
                const event = await alertEscalationService.escalateAlert(alertId, 'critical', 'Auto-escalated after 30 minutes');
        ...
        it('should throw error for non-existent alert', async () => {
            await expect(alertEscalationService.escalateAlert('non-existent-id', 'critical', 'Test')).rejects.toThrow();
        });
    });
    describe('getEscalationHistory', () => {
        it('should return escalation history for an alert', async () => {
            ...
            try {
                ...
                const alertResult = await client.query(`INSERT INTO alerts (
            battery_system_id, facility_id, type, severity, status, message
          ) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`, ['battery-1', 'default', 'Temperature High', 'medium', 'active', 'Temperature rising']);
