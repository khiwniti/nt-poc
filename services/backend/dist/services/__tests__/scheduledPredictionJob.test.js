...
describe('ScheduledPredictionJob', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        vi.mocked(mlModel.getModel).mockReturnValue(mockModel);
    ...
    describe('Job Initialization', () => {
        ...
        it('should throw error if ML model initialization fails', async () => {
            vi.mocked(mlModel.initializeModel).mockRejectedValueOnce(new Error('Model init failed'));
    ...
    describe('Job Execution', () => {
        ...
        it('should store predictions with correct RUL mapping', async () => {
            ...
            await job.triggerManually();
            expect(pool.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO rul_predictions'), expect.arrayContaining([
                'battery-1',
                14, // RUL for '14d' risk
    ...
    describe('Job Status and Metrics', () => {
        ...
        it('should prevent concurrent job execution', async () => {
            vi.mocked(pool.query).mockImplementation(() => new Promise(resolve => setTimeout(() => resolve({ rows: [], rowCount: 0 }), 100)));
    ...
    describe('Feature Extraction', () => {
        it('should extract correct features from battery data', async () => {
            ...
            await job.triggerManually();
            expect(mockModel.predict).toHaveBeenCalledWith('battery-1', {
                sohDelta: -0.03,
                anomalyCount: 4,
                tempMax: 40,
                voltageMin: 3.4,
