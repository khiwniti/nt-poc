 ...
 * }
 */
router.post('/shap/waterfall', async (req, res) => {
    try {
        const { batterySystemId, features, predictionValue, predictionType = 'RUL' } = req.body;
        // Validation
        ...
        const response = await axios.post(`${ML_SERVICE_URL}/explain/waterfall`, {
 ...
 * }
 */
router.post('/shap/force', async (req, res) => {
    try {
        const { batterySystemId, features, predictionValue, predictionType = 'RUL' } = req.body;
        // Validation
        if (!batterySystemId || !features || predictionValue === undefined) {
        ...
        const response = await axios.post(`${ML_SERVICE_URL}/explain/force`, {
            battery_system_id: batterySystemId,
            features,
            prediction_value: predictionValue,
            prediction_type: predictionType
...
router.post('/explanation', async (req, res) => {
    try {
        const { batterySystemId, features, predictionValue, predictionType = 'RUL', topN = 5 } = req.body;
        // Validation
        if (!batterySystemId || !features || predictionValue === undefined) {
            return res.status(400).json({
                error: 'Missing required fields: batterySystemId, features, predictionValue'
        ...
        const response = await axios.post(`${ML_SERVICE_URL}/explain/text`, {
            battery_system_id: batterySystemId,
            features,
            prediction_value: predictionValue,
            prediction_type: predictionType,
...
router.post('/export', async (req, res) => {
    try {
        ...
        const response = await axios.post(`${ML_SERVICE_URL}/explain/export`, {
            battery_system_id: batterySystemId,
            features,
            prediction_value: predictionValue,
            prediction_type: predictionType
 ...
 */
router.get('/prediction/:predictionId/explain', async (req, res) => {
    try {
