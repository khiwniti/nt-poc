...
import { haversineDistance, estimateDrivingTime, formatDistance, formatDuration, getDirectionsUrl, } from '../utils/distance.js';
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query(`SELECT id, name, location, timezone, total_zones as "totalZones", status, created_at, updated_at
...
router.get('/nearby/:id', async (req, res) => {
    try {
        ...
        // Get origin facility
        const originFacility = await pool.query('SELECT id, name, latitude, longitude FROM facilities WHERE id = $1', [id]);
    ...
    `;
        const params = [origin.longitude, origin.latitude, id];
        let paramIndex = 4;
        if (maxDistance !== null) {
    ...
    `;
        ...
        const nearbyFacilities = result.rows.map((facility) => ({
            ...
            directionsUrl: getDirectionsUrl(origin.latitude, origin.longitude, facility.latitude, facility.longitude),
        }));
        const response = {
            origin: {
                id: origin.id,
        ...
        if (includeMatrix) {
            const allFacilities = await pool.query(`SELECT id, name, latitude, longitude
         FROM facilities
