...
router.get('/:batteryId', async (req, res) => {
    try {
        ...
        const batteryCheck = await pool.query('SELECT id FROM battery_systems WHERE id = $1', [batteryId]);
      ...
      LIMIT $2 OFFSET $3`, [batteryId, limit, offset]);
        // Get total count for pagination
        const countResult = await pool.query('SELECT COUNT(*) as total FROM rul_predictions WHERE battery_system_id = $1', [batteryId]);
 ...
 */
router.get('/:batteryId/latest', async (req, res) => {
    try {
        const { batteryId } = req.params;
        const result = await pool.query(`SELECT
        id,
...
router.post('/', async (req, res) => {
    try {
        const { batterySystemId, predictedRUL, confidence, modelVersion, features, } = req.body;
        // Validation
        ...
        const batteryCheck = await pool.query('SELECT id FROM battery_systems WHERE id = $1', [batterySystemId]);
        if (batteryCheck.rows.length === 0) {
            return res.status(404).json({ error: 'Battery system not found' });
        }
        // Insert prediction
        const result = await pool.query(`INSERT INTO rul_predictions (
        battery_system_id,
        predicted_rul,
        confidence,
        model_version,
      ...
      RETURNING
        ...
        model_version,
        features,
        created_at`, [batterySystemId, predictedRUL, confidence, modelVersion, JSON.stringify(features || {})]);
        const prediction = mapRowToPrediction(result.rows[0]);
        const response = {
            data: prediction,
        };
...
router.delete('/cleanup', async (req, res) => {
    try {
        const result = await pool.query(`DELETE FROM rul_predictions
