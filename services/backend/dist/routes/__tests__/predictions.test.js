...
describe('RUL Predictions API', () => {
    ...
    describe('DELETE /api/v1/predictions/cleanup', () => {
      ...
      `, [oldPredictionId, testBatteryId]);
            ...
            const checkResult = await pool.query('SELECT id FROM rul_predictions WHERE id = $1', [oldPredictionId]);
            expect(checkResult.rows.length).toBe(0);
        });
        it('does not remove predictions within 90 days', async () => {
            const recentCount = await pool.query('SELECT COUNT(*) as count FROM rul_predictions WHERE battery_system_id = $1 AND created_at >= NOW() - INTERVAL \'90 days\'', [testBatteryId]);
            await request(app)
                .delete('/api/v1/predictions/cleanup')
                .set('Authorization', `Bearer ${authToken}`)
                .expect(200);
            const afterCount = await pool.query('SELECT COUNT(*) as count FROM rul_predictions WHERE battery_system_id = $1 AND created_at >= NOW() - INTERVAL \'90 days\'', [testBatteryId]);
