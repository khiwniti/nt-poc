...
from app.models.sensor_data import SensorReading, BatteryState, BatteryMetrics
...
class SimulatorSensor(SensorInterface):
    ...
    async def get_metrics(self, battery_system_id: str) -> Dict[str, Any]:
        ...
        metrics = BatteryMetrics(
            ...
            health_indicators={
                ...
                "estimated_eol_cycles": int((state.soh - 70) / self._soh_decay_rate) if state and self._soh_decay_rate > 0 else 0
            }
        )
    ...
    def _get_or_create_state(self, battery_system_id: str) -> BatteryState:
        ...
        if battery_system_id not in self._battery_states:
            # Initialize with realistic starting values
            self._battery_states[battery_system_id] = BatteryState(
                battery_system_id=battery_system_id,
            ...
            logger.debug(f"Created new state for {battery_system_id}")
