...
import { batterySystemsApi, SensorReading } from '../../api/batterySystems';
    ...
    <div className="w-full bg-white rounded-xl p-4 border border-gray-100 shadow-sm relative overflow-hidden group">
      ...
      <div className="relative h-[100px] w-full">
        ...
        >
          ...
          />
          <path
            d={`M ${padding},${height} ${points.split(' ')[0]} ${points} L ${width - padding},${height} Z`}
            fill={`url(#grad-${label})`}
            stroke="none"
          />
          <polyline
        ...
        <div className="absolute top-0 right-0 text-[10px] text-gray-400 font-mono bg-white/80 px-1 rounded">
          {max}
          {unit}
        </div>
        <div className="absolute bottom-0 right-0 text-[10px] text-gray-400 font-mono bg-white/80 px-1 rounded">
          {min}
          {unit}
        </div>
      </div>
        ...
        {activeTab === 'overview' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-1 space-y-4">
              <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-bold uppercase text-blue-500 flex items-center gap-2">
                    <Zap size={14} /> แรงดันไฟฟ้า
                  </span>
                  <span className="text-xs text-gray-400">Target: {targetVoltage}V</span>
                </div>
                <div className="text-3xl font-mono font-bold text-slate-800">
                  {data.voltage.toFixed(3)} <span className="text-lg text-gray-400">V</span>
                </div>
              </div>
              <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-bold uppercase text-red-500 flex items-center gap-2">
                    <Thermometer size={14} /> อุณหภูมิ
                  </span>
                  <span className="text-xs text-gray-400">Limit: 30°C</span>
                </div>
                <div className="text-3xl font-mono font-bold text-slate-800">
                  {data.temperature.toFixed(1)} <span className="text-lg text-gray-400">°C</span>
                </div>
              </div>
              <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-bold uppercase text-purple-500 flex items-center gap-2">
                    <Activity size={14} /> ความต้านทานภายใน
                  </span>
              ...
              <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden">
                <div className="relative z-10">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold uppercase text-emerald-500 flex items-center gap-2">
                      <ClipboardList size={14} /> อายุการใช้งานที่เหลือ (RUL)
                    </span>
            ...
            <div className="lg:col-span-2 space-y-4">
              {isLoadingHistory && sensorHistory.length === 0 ? (
                <div className="bg-white rounded-xl p-8 border border-gray-100 shadow-sm flex flex-col items-center justify-center text-gray-400">
                  <div className="w-8 h-8 border-4 border-blue-200 border-t-blue-600 rounded-full animate-spin mb-4"></div>
                  <span className="text-sm">กำลังโหลดข้อมูลเซ็นเซอร์...</span>
                </div>
              ) : (
                <>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden text-slate-800">
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-gray-100">
                ...
                {(data.historyLog && data.historyLog.length > 0
                  ? data.historyLog
                  : [
                      {
                        date: '2024-05-12',
                      ...
                      },
                      {
                        ...
                        result: 'Pass',
                        tech: 'Wichai R.',
                        notes: 'Annual discharge test',
                      },
                    ]
                ).map((log: MaintenanceLog, i: number) => (
                  <tr key={i} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 text-gray-800 font-mono">{log.date}</td>
                    <td className="px-6 py-4 text-gray-600">
                      {log.type === 'Impedance' ? 'ทดสอบความต้านทาน' : 'ทดสอบการคายประจุ'}
                    </td>
