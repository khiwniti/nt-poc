...
export const SparePartsManager: React.FC = () => {
    ...
    useEffect(() => {
        ...
        ]).then(([partsData, suppliersData, ordersData]) => {
          setParts(partsData);
    ...
    const handleCreatePO = () => {
        ...
        setTimeout(() => {
            ...
            db.createPurchaseOrder(createdPO).then(() => {
              return Promise.all([
                db.getPurchaseOrders(),
                db.getSpareParts()
              ]);
            }).then(([ordersData, partsData]) => {
              setPurchaseOrders(ordersData);
              setParts(partsData);
    ...
    return (
        <div className="w-full h-full flex overflow-hidden bg-slate-50">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-end mb-6 gap-4">
                        <div>
                            <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-3">
                                <Package className="text-slate-400" /> คลังสินค้าและอะไหล่
                            </h1>
                        ...
                        <div className="flex items-center bg-white p-1 rounded-lg border border-slate-200 shadow-sm">
                            ...
                            >
                                <FileText size={14} /> ใบสั่งซื้อ (PO)
                            </button>
                            <button 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                        <Card className="p-4 bg-white border-slate-200">
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">มูลค่าคลังสินค้า</p>
                        ...
                        <Card className={`p-4 border-slate-200 ${lowStockCount > 0 ? 'bg-amber-50/20 border-amber-200' : 'bg-white'}`}>
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">แจ้งเตือนสินค้าต่ำ</p>
                        ...
                        <Card className={`p-4 border-slate-200 ${outOfStockCount > 0 ? 'bg-red-50/20 border-red-200' : 'bg-white'}`}>
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">สินค้าหมด</p>
                        ...
                        <Card className="p-4 bg-white border-slate-200">
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">รอรับสินค้า (PO)</p>
                    ...
                    {activeTab === 'inventory' && (
                        <div className="flex items-center gap-3 bg-white p-3 rounded-xl border border-slate-200 shadow-sm mb-4">
                            <div className="relative group flex-1">
                                ...
                                <input 
                                    type="text"
                                    placeholder="ค้นหาชื่ออะไหล่, SKU, หรือซัพพลายเออร์..."
                                    value={searchTerm}
                        ...
                        <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                            <table className="w-full text-left text-sm">
                                <thead className="bg-slate-50 border-b border-slate-200">
                                    <tr>
                                        <th className="px-6 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider">ข้อมูลอะไหล่</th>
                                        <th className="px-6 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider">สถานที่เก็บ</th>
                                        <th className="px-6 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider w-48">ระดับสต็อก</th>
                                        <th className="px-6 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider text-right">ต้นทุนต่อหน่วย</th>
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4">
                                                ...
                                                {part.currentStock <= part.minStock && (
                                                    <div className="text-[9px] text-red-500 font-bold mt-1">ต่ำกว่าจุดสั่งซื้อ ({part.minStock})</div>
                                                )}
                                            </td>
                    ...
                    {activeTab === 'orders' && (
                        <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
                            <table className="w-full text-left text-sm">
                                <thead className="bg-slate-50 border-b border-slate-200">
                                    <tr>
                                        <th className="px-6 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider">เลขที่ PO</th>
                                        <th className="px-6 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider">ซัพพลายเออร์</th>
                                        <th className="px-6 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider">วันที่</th>
                                        <th className="px-6 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider">ยอดรวม</th>
                                        <th className="px-6 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider">สถานะ</th>
                                        <th className="px-6 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider">ผู้ขอซื้อ</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100">
                                        ...
                                        return (
                                            <tr key={po.id} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    ...
                                                    <div className="text-[10px] text-slate-400">{po.items.length} รายการ</div>
                                                </td>
                                                <td className="px-6 py-4 text-xs text-slate-600">
                                                    {po.createdDate.toLocaleDateString('th-TH')}
                                                </td>
                            ...
                            {suppliers.map(supplier => (
                                <div key={supplier.id} className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-all group">
                                    ...
                                    <div className="pt-4 border-t border-slate-100 flex justify-between items-center">
                                        <span className="text-[10px] text-slate-400">{supplier.activeContracts} สัญญาที่ใช้งานอยู่</span>
                                        <button className="text-xs text-blue-600 font-bold hover:underline">ดูแคตตาล็อก</button>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <>
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                                ...
                                <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                                    <h3 className="text-xs font-bold text-slate-900 uppercase tracking-widest mb-3 flex items-center gap-2">
                                        <Box size={14} /> สถานะสต็อก
                                    </h3>
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-3xl font-bold text-slate-800">{selectedPart.currentStock}</span>
                                        <span className="text-sm text-slate-500">{selectedPart.unit} คงเหลือ</span>
                                    </div>
                                    <div className="space-y-2">
                                        <div className="flex justify-between text-xs text-slate-500">
                                            <span>จุดสั่งซื้อ: {selectedPart.minStock}</span>
                                            <span>ความจุสูงสุด: {selectedPart.maxStock}</span>
                                        </div>
                                        <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                                            <div 
                                                className={`h-full rounded-full ${selectedPart.currentStock <= selectedPart.minStock ? 'bg-red-500' : 'bg-blue-500'}`} 
                                                style={{ width: `${Math.min(100, (selectedPart.currentStock / selectedPart.maxStock) * 100)}%` }}
                                            ></div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="p-3 bg-white border border-slate-200 rounded-lg">
                                        <span className="text-[10px] text-slate-400 uppercase font-bold block mb-1">ซัพพลายเออร์</span>
                                        <span className="text-xs font-bold text-slate-700">{selectedPart.supplier}</span>
                                    </div>
                                    <div className="p-3 bg-white border border-slate-200 rounded-lg">
                                        <span className="text-[10px] text-slate-400 uppercase font-bold block mb-1">เวลานำส่ง</span>
                                        <span className="text-xs font-bold text-slate-700">{selectedPart.leadTimeDays} วัน</span>
                                    </div>
                                    <div className="p-3 bg-white border border-slate-200 rounded-lg">
                                        <span className="text-[10px] text-slate-400 uppercase font-bold block mb-1">ต้นทุนต่อหน่วย</span>
                                        <span className="text-xs font-mono font-bold text-slate-700">฿{selectedPart.costPerUnit}</span>
                                    </div>
                                    <div className="p-3 bg-white border border-slate-200 rounded-lg">
                                        <span className="text-[10px] text-slate-400 uppercase font-bold block mb-1">ใช้งานล่าสุด</span>
                                ...
                                <div>
                                    <h3 className="text-xs font-bold text-slate-900 uppercase tracking-widest mb-3 flex items-center gap-2">
                                        <RefreshCw size={14} /> ใช้ได้กับรุ่น
                                    </h3>
                                ...
                                <div>
                                    <h3 className="text-xs font-bold text-slate-900 uppercase tracking-widest mb-3 flex items-center gap-2">
                                        <BarChart3 size={14} /> การเคลื่อนไหวล่าสุด
                                    </h3>
                                    <div className="space-y-3 border-l-2 border-slate-100 ml-2 pl-4 relative">
                                        {[1,2,3].map((i) => (
                                            <div key={i} className="text-xs relative">
                                                ...
                                                <p className="font-bold text-slate-700">ใช้ในใบงาน #WO-2025-08{i}</p>
                                                <p className="text-slate-400 text-[10px]">1{5-i} พ.ค. 2568 • -2 หน่วย</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="p-4 border-t border-slate-200 bg-white space-y-2">
                                <button className="w-full py-2 bg-nt-dark text-white rounded-lg text-xs font-bold hover:bg-slate-800 transition-colors flex items-center justify-center gap-2">
                                    <Truck size={14} /> สร้างใบสั่งซื้อ (PO)
                                </button>
            ...
            {/* Create PO Modal */}
            <Modal isOpen={isPOModalOpen} onClose={() => setIsPOModalOpen(false)} title="สร้างใบสั่งซื้อ (Purchase Order)">
                <div className="p-6">
                    <div className="space-y-4">
                        <div>
                            <label className="block text-xs font-bold text-slate-500 uppercase mb-1">ซัพพลายเออร์</label>
                            <select 
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-500 uppercase mb-1">รายการสินค้า</label>
                            <div className="space-y-2">
                        ...
                        {newPO.items.length > 0 && (
                            <div className="pt-4 border-t border-slate-100 flex justify-between items-center">
                                <span className="text-xs text-slate-500">จำนวนรายการ: {newPO.items.length}</span>
                    ...
                    <div className="flex justify-end gap-3 mt-6">
                        <button onClick={() => setIsPOModalOpen(false)} className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded text-xs font-bold">ยกเลิก</button>
                        <button 
